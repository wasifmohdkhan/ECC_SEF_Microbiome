---
title: "Maaslin2 on 16S and ITS with NO confounders Genus"
...
new_taxa_names_16S_Genus <- gsub("g__", "B_", taxa_names(Plaque538_16S_Genus_ps))
...
head(taxa_names(otu_table(Plaque538_16S_Genus_ps)))
...
```{r}
# Extract and modify taxa names
new_taxa_names_ITS_Genus <- gsub("g__", "F_", taxa_names(Plaque538_ITS_Genus_ps))

# Update taxa names in-place in the phyloseq object
taxa_names(Plaque538_ITS_Genus_ps) <- new_taxa_names_ITS_Genus

# Confirm that the 's__' prefix has been removed
otu_table(Plaque538_ITS_Genus_ps) %>% as.data.frame()
```
...
```{r}
Maaslin2_output_dir = here("Plaque538_Combined/Plaque538_16S/Plaque538_16S_A4_LM/Maaslin2_240216")
...
Maaslin2_Heatmap_Genus_without_confounders =  heatmap_object_gg +
  theme_minimal() +  # Using a minimal theme as a base
   theme(
    ...
    axis.title.x = element_text(size = 18, hjust = .30),  # Move x-axis title slightly left
    axis.title.y = element_text(size = 18, angle = 90),  # Customize y-axis title size and angle
  ) +
  # annotate("text", x = 1, y = 1, label = "Coefficient Estimates", hjust = 1, vjust = 1.2, size = 5, colour = "black") +
  #  annotate(
  #   "text", x = 1, y = 1,
  #   label = expression(paste("*", italic("q"), "-value < 0.01")),
  #   hjust = 1.2, vjust = 16, size = 5, colour = "black"
  # ) +
  # annotate(
  #   "text", x = 1, y = 1,
  #   label = expression(paste("**", italic("q"), "-value < 0.001")),
  #   hjust = 1.1, vjust = 18, size = 5, colour = "black"
  # ) +
  labs(
    x = "Host Variables",  # Replace with your actual x-axis title
    y = "Microbial Genus (Bacteria and Fungi)",  # Replace with your actual y-axis title
  )

Maaslin2_Heatmap_Genus_without_confounders

saveRDS(Maaslin2_Heatmap_Genus_without_confounders,
        here("Plaque538_Combined/Plaque538_16S/Plaque538_16S_A4_LM/Maaslin2_Heatmap_Genus_without_confounders.rds"))

# ggsave(here("Plaque538_Combined/Plaque538_16S/Plaque538_16S_A4_LM/Maaslin2_Heatmap_Genus_without_confounders.png"),
