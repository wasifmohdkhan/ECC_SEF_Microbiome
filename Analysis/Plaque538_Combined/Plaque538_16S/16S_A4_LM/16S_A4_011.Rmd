---
title: "Linda on 16S and ITS with confounders Species"
...
```{r}
Plaque538_ITS_AllOTU_5perc_ps =   readRDS(here("Plaque538_Combined/Plaque538_ITS/Plaque538_ITS_A2_pseq_objects/Plaque538_ITS_AllOTU_5perc_ps_17_vars_240306.Rds"))
...
We will apply LinDA separately on 16S and ITS data and later combine the output to get a single heatmap

```{r}
confounders = c("Sex", "Age", "Urban_status")
confounders_string = paste(confounders, collapse = " + ")
```


## Linda on 16S with confounders
```{r}
...
# Iterate through each variable to run linda and filter results
for(var in col_without_confounders) {
  
  # Construct the formula for the current variable
  current_formula <- paste("~", var, "+", confounders_string)
  
  # Run linda analysis
  linda_res <- MicrobiomeStat::linda(feature.dat = otu_table(pseq),
                                     meta.dat = data.frame(sample_data(pseq)),
                                     formula = current_formula,
                                     n.cores = 40)
  
  # Filter and transform the output data frame
  filtered_df <- linda_res[["output"]][1] %>% 
                 data.frame() %>% 
                 rownames_to_column("OTUs") %>% 
                 rename_all(~sub(".*\\.", "", .)) %>% 
                 filter(abs(log2FoldChange) >= 0.75 & padj <= 0.05) %>% 
                 dplyr::select(OTUs, log2FoldChange, padj)
  
  # Rename the log2FoldChange and padj columns to the current variable
  filtered_df_lfc <- filtered_df %>% dplyr::select(OTUs, log2FoldChange) %>% 
                     rename(!!var := log2FoldChange)
                     
  filtered_df_padj <- filtered_df %>% dplyr::select(OTUs, padj) %>% 
                      rename(!!var := padj)
  
  # Store in lists
  list_lfc_16S[[var]] <- filtered_df_lfc
  list_padj_16S[[var]] <- filtered_df_padj
}

final_df_lfc_16S <- list_lfc_16S[[1]]
for(i in 2:length(list_lfc_16S)) {
  final_df_lfc_16S <- full_join(final_df_lfc_16S, list_lfc_16S[[i]], by = "OTUs")
}

final_df_padj_16S <- list_padj_16S[[1]]
for(i in 2:length(list_padj_16S)) {
  final_df_padj_16S <- full_join(final_df_padj_16S, list_padj_16S[[i]], by = "OTUs")
}

final_df_coef_16S = final_df_lfc_16S
final_df_qval_16S = final_df_padj_16S
...
for(var in col_without_confounders) {
  ...
  # Store in lists
  list_lfc_ITS[[var]] <- filtered_df_lfc
  list_padj_ITS[[var]] <- filtered_df_padj
}

final_df_lfc_ITS <- list_lfc_ITS[[1]]
for(i in 2:length(list_lfc_ITS)) {
  final_df_lfc_ITS <- full_join(final_df_lfc_ITS, list_lfc_ITS[[i]], by = "OTUs")
}

final_df_padj_ITS <- list_padj_ITS[[1]]
for(i in 2:length(list_padj_ITS)) {
  final_df_padj_ITS <- full_join(final_df_padj_ITS, list_padj_ITS[[i]], by = "OTUs")
}

final_df_coef_ITS = final_df_lfc_ITS
final_df_qval_ITS = final_df_padj_ITS
```
...
ggsave(here("Plaque538_Combined/Plaque538_16S/Plaque538_16S_A4_LM/LinDa_Heatmap_Species_with_confounders.png"),
      plot = heatmap_object_gg2, 
       width = 28,
       height = 41,
       units = "cm",
       dpi = 600)
ggsave(here("Plaque538_Combined/Plaque538_16S/Plaque538_16S_A4_LM/LinDa_Heatmap_Species_with_confounders.tiff"),
      plot = heatmap_object_gg2, 
       width = 28,
       height = 41,
       units = "cm",
...
```{r}
final_df_coef_16S_ITS_2 = final_df_coef_16S_ITS %>% 
  select(OTUs, ECC_status) %>% 
  rename("Coefficients" = ECC_status)


final_df_qval_16S_ITS_2 = final_df_qval_16S_ITS %>% 
  select(OTUs, ECC_status) %>% 
  rename("q_values" = ECC_status)


final_df_coef_qval_16S_ITS = full_join(final_df_coef_16S_ITS_2, final_df_qval_16S_ITS_2, by = "OTUs") %>% 
...
coef_plot <- ggplot(data, aes(y = Shortened_OTUs, x = Coefficients, fill = Sign)) +
  ...
  labs(x = "LinDA Coefficients", y = "Microbial Species") +
  theme_bw() +
  theme(axis.text.x = element_text(colour = "black", family = "Ariel"),
...
pval_plot <- ggplot(data, aes(y = Shortened_OTUs, x = log_q_values, fill = Sign)) +
  ...
  scale_x_break(c(13, 40), scales = 0.2, ticklabels = c(0,5, 10, 40, 41), space = 0.01) +
  theme(axis.text.x = element_text(), axis.ticks.y = element_blank())

combined_plot <- coef_plot + pval_plot + 
                 plot_layout(ncol = 2, widths = c(2, 1), axes = "collect") + # Set the relative widths+ 
...
# Save the plot
ggsave(here("Plaque538_Combined/Plaque538_16S/Plaque538_16S_A4_LM/LinDA_taxa_barplot.png"),
       plot = combined_plot,
...
coef_plot <- ggplot(data, aes(y = Shortened_OTUs, x = Coefficients, fill = Sign)) +
  ...
  scale_fill_manual(values = c("ECC" = "darkred", "CF" = "darkblue")) +
  labs(x = "LinDA Coefficients", y = "Microbial Species") +
  scale_x_break(c(4, 4.1), scales = 0.00001, ticklabels = c(-3, -2, -1, 0, 1, 2, 3), space = 0.01) +
  #use  scale_x_break in both the plots otherwise they wont be aligned
  # c(4, 4.1), scales = 0.00001 this provides a dummy break which is not visible in the final plot
  theme_bw() +
  theme(axis.text.x = element_text(colour = "black", family = "Ariel"),
        axis.text.y = element_text(colour = "black", family = "Ariel", size = 10),
        axis.title.y = element_text(colour = "black", family = "Ariel", size = 10),
...
pval_plot <- ggplot(data, aes(y = Shortened_OTUs, x = log_q_values, fill = Sign)) +
  ...
  labs(x = "-log10(q-value)", y = "") +  # Empty y-axis label to avoid duplication
  theme_bw() +
  # scale_x_continuous(limits = c(0, 43), breaks = c(0, 5, 10, 15, 40, 43)) +
  scale_x_break(c(13, 40), scales = 0.2, ticklabels = c(0,5, 10, 40, 41), space = 0.1) +
  theme(axis.text.x = element_text(colour = "black", family = "Ariel"),
        axis.text.y = element_blank(), # Hide y-axis text for the second plot
        axis.ticks.y = element_blank())

# Combine the plots with a common y-axis
combined_plot <- free(coef_plot) + free(pval_plot) + 
                 plot_layout(ncol = 2, widths = c(2, 1), guides = "collect") + 
                 plot_annotation(tag_levels = 'A')

# Print the combined plot with custom theme settings
print(combined_plot) +
...
# Save the plot
ggsave(here("Plaque538_Combined/Plaque538_16S/Plaque538_16S_A4_LM/LinDA_taxa_barplot.tiff"),
       plot = combined_plot,
       width = 10, height = 10, dpi = 300)
```
